...
export class RedisRepository {
  ...
  async HGET<T = null>(
    ...
    if (log) this.logger.info(`Getting data from redis ${formattedKey} ${key}`);
  ...
  async HGETALL<T>(
    ...
    if (log) this.logger.info(`Getting all data from redis ${formattedKey}`);
  ...
  async HLEN(
    ...
    if (log) this.logger.info(`Getting length from redis ${formattedKey}`);
  ...
  async HKEYS(
    ...
    if (log) this.logger.info(`Getting keys from redis ${formattedKey}`);
  ...
  async getEntity(category: RedisCategories, key: string, log = false) {
    try {
      if (log) this.logger.info(`Getting data from redis ${category} ${key}`);
  ...
  async deleteEntity(category: RedisCategories, key: string, log = false) {
    try {
      if (log) this.logger.info(`Deleting data from redis ${category} ${key}`);
  ...
  async deleteAllFromCategory(category: RedisCategories, log = false) {
    try {
      if (log) this.logger.info(`Deleting all data from redis ${category}`);
  ...
  async deleteAllFromCategoryHash(category: RedisCategories, log = false) {
    try {
      if (log)
  ...
  async getAllFromCategory<T>(
    ...
    try {
      if (log) this.logger.info(`Getting all data from redis ${category}`);
      const keys = await this.redis.keys(`${category}:*`);
